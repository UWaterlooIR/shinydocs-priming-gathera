"""
Django settings for Gathera project.
...
ADMINS = [
    ("""Mustafa S""", 'mabualsaud@outlook.com'),
]
...
ACCOUNT_SIGNUP_FORM_CLASS = 'web.allauth.forms.SignupForm'
...
LOGGING = {
    ...
    'formatters': {
        'verbose': {
            'format': '[%(asctime)s.%(msecs).06d] %(levelname)s [%(name)s.%(funcName)s:%(lineno)d] %(message)s',
            'datefmt': '%Y-%m-%d %H:%M:%S'
        },
    },
    'handlers': {
...
DOCUMENTS_URL = env.str("DOCUMENT_URL", default="http://nginx:9000/doc")
PARA_URL = env.str("PARA_URL", default="http://nginx:9000/para")
